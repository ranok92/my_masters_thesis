...
\textcolor{red}{apprenticeship-learning-2004} shows that solving the Bellman equations for an analytic solution to obtain the reward R is under-constrained. \textcolor{red}{ziebart} addresses this by introducing an entropy-based constraint \textcolor{red}{entropy formulation paper} on the distribution of the trajectories: the probability of the occurrence of a trajectory is directly proportional to the reward it receives.
\begin{align}
...
We solve the MDP in the inner loop using an actor-critic method \textcolor{red}{cite A2C}, which we will describe in detail in the next section.
...
\end{align}
The baseline can be calculated in various ways. One of the resulting algorithms is the A2C or the Advantage actor-critic algorithm, which we use in our pipeline.
The A2C method:
...
\subsubsection*{Speed}
The current speed of the agent is quantized and represented in the form of a $6 \times 1$ one-hot vector. The thresholds of the quantization are mentioned in \autoref{tab:speed-quantization}
...
Taking inspiration from previous works in this field \cite{fahad-et-al} \cite{vasquez-et-al}, we use spatial bins to effectively divide the region surrounding the agent into discrete segments and calculate a 'risk' metric for each of these bins.
